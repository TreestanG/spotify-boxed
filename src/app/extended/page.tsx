...
import { ExtendedStats, SpotifyData, } from "@/types/types";
...
export default function Home() {
    ...
    const [spotifyStats, setSpotifyStats] = useState<ExtendedStats>()
    ...
    const handleSubmit = async (e: any) => {
        ...
        setLoading(true)
        const res = await fetch("/api/upload-extended", {
            method: "POST",
            body: formData,
        })
        ...
        <main className={cn("min-h-screen w-screen flex justify-center lg:p-0", `${stats ? '' : " items-center"}`)}>
            {
                loading ? (
                    ...
                    <div className="w-full flex flex-col">
                        <div className="w-full h-auto lg:h-[20rem] bg-gradient-to-t from-spotify-header-bottom to-spotify-header-top flex flex-col lg:pl-20 gap-8 py-6 p-6 justify-center">
                            <Link className="text-white underline text-lg" href="/">Home</Link>

                            <h1 className="text-white text-3xl lg:text-8xl font-extrabold">Extended Streaming History </h1>
                        </div>
                        <div className="flex text-white px-6 lg:pl-40 gap-4 text-lg lg:text-xl w-full justify-center lg:justify-start pt-20 ">
                            ...
                            <p onClick={() => handleTimeChange('allTime')} className={cn("hover:underline underline-offset-8 cursor-pointer decoration-spotify-green", time === "allTime" ? "underline" : "")}>All Time</p>
                            ...
                            <div className="lg:w-10/12 w-full  flex flex-col gap-20">
                                ...
                                <div className="flex gap-4 flex-col max-w-full pb-20">
                                    ...
                                    <div className="flex gap-8 flex-col lg:flex-row">
                                        <BlurFade className="w-full lg:w-1/4" delay={.15}><Leaderboard title="Top Artists" measures="Plays" data={spotifyStats!.artistAppearances} /></BlurFade>
                                        <BlurFade className="w-full lg:w-1/4" delay={.3}><Leaderboard title="Top Songs" measures="Plays" data={spotifyStats!.songAppearances} /></BlurFade>
                                        <BlurFade className="w-full lg:w-1/4" delay={.45}><Leaderboard title="Top Albums" measures="Plays" data={Object.fromEntries(spotifyStats!.albumAppearances.map(c => [`${c.albumName} - ${c.albumArtist}`, c.appearances]))} /></BlurFade>
                                        <BlurFade className="w-full lg:w-1/4" delay={.45}><Leaderboard title="Artists By the Numbers" measures="Minutes" data={Object.fromEntries(spotifyStats!.minutesPerArtist.map(c => [c.artist, c.minutes]))} /></BlurFade>
                        ...
                        <form onSubmit={handleSubmit} className="flex flex-col">
                            ...
                            <div className="h-[75vh] lg:h-auto flex items-center justify-center flex-col w-full">
                                <label className="flex flex-col items-center justify-center w-full">
                                    <span className="text-4xl lg:text-5xl font-bold text-white">Spotify Boxed - Extended</span>
                                ...
                                <p className="text-lg pt-4 text-spotify-text">The extended version tends to take around <span className="font-semibold text-white">3-4 weeks</span> to send. </p>
                                <p className="text-lg text-spotify-text">If you don&apos;t have your data yet, click on &quot;Toggle Demo&quot; and click submit.</p>
                            ...
                            <div className="text-spotify-text pt-4 flex flex-col justify-start w-full">
                                ...
                                <p className="">4. Scroll down to <span className="font-semibold">Download your data</span></p>
                                <p className="">5. Follow instructions under <span className="font-semibold">Extended streaming history</span></p>
                                <p className="">6. Wait for up to 30 days for the email containing your data and press <span className="font-semibold">download</span> once ready.</p>
                                <p className="">7. Upload the zip file here and enjoy :) </p>
                                <p className="">Note: the data here will only contain music you&apos;ve listened to in your entire <span className="text-white font-semibold">entire Spotify history. </span></p>
